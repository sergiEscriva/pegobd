...
class ELM327Communication {
  ...
  static Future<bool> initializeELM327(BluetoothConnection connection) async {
    ...
    while (retryCount < MAX_RETRIES) {
      try {
        print(
          "🔧 Intento ${retryCount + 1}/$MAX_RETRIES: Inicializando ELM327...",
        );
        _logger.info(
          "Intento ${retryCount + 1}/$MAX_RETRIES: Inicializando ELM327",
          tag: 'ELM327',
        ...
        if (success) {
          ...
          _logger.warning(
            "Inicialización falló en intento ${retryCount + 1}",
            tag: 'ELM327',
          );
          retryCount++;
        ...
        _logger.error(
          "Error en inicialización ELM327",
          error: e,
    ...
    _logger.critical(
      "Inicialización ELM327 falló después de $MAX_RETRIES intentos",
      tag: 'ELM327',
    );
    return false;
  }
  ...
  static Future<bool> _performInitialization(
  ...
  static Future<String> sendCommand(
    ...
    try {
      ...
      _logger.error(
        "Error enviando comando ELM327: $command",
        error: e,
        stackTrace: stackTrace,
        tag: 'ELM327',
      );
