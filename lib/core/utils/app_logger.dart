...
class AppLogger {
  ...
  Future<void> _rotateLogIfNeeded() async {
    ...
    if (fileSize > MAX_LOG_SIZE) {
      ...
      await _writeLog('📱 Log rotado - Tamaño anterior: ${fileSize / 1024} KB',
          LogLevel.INFO);
    }
  }
  ...
  Future<void> _writeLog(String message, LogLevel level) async {
    ...
    try {
      final timestamp = DateFormat('yyyy-MM-dd HH:mm:ss.SSS').format(
          DateTime.now());
  ...
  Future<void> error(String message,
      {dynamic error, StackTrace? stackTrace, String? tag}) async {
    final logMessage = tag != null ? '[$tag] $message' : message;
