...
class OBDSensorDetector {
  ...
  static Future<Map<String, bool>> detectAvailableSensors(
    ...
    try {
      ...
      List<String> priorityPids = ['04', '05', '0C', '0D', '0F', '10', '11', '2F', '5C'];
  ...
  static Future<Map<String, bool>> _querySupportedPids(BluetoothConnection connection) async {
  ...
  static void _parseSupportedPidsResponse(String response, int baseValue, Map<String, bool> supportedPids) {
    if (response.isEmpty || response.contains('ERROR') || response.contains('NO DATA')) {
      return;
    }
    ...
    try {
      ...
      if (parts.length >= 6) {
        ...
        for (int i = 0; i < 32; i++) {
          if ((bitmap & (1 << (31 - i))) != 0) {
            String pid = (baseValue + i).toRadixString(16).toUpperCase().padLeft(2, '0');
  ...
  static Future<bool> _testSinglePid(BluetoothConnection connection, String pid) async {
    try {
  ...
  static Future<String> _sendOBDCommand(BluetoothConnection connection, String command) async {
