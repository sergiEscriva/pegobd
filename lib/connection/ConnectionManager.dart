...
class ConnectionManager {
  ...
  Future<void> connect(BluetoothDevice device) async {
    ...
    try {
      _logger.info('Iniciando conexión a: ${device.name} (${device.address})', tag: 'CONNECTION');
      ...
      _logger.error('Error en conexión', error: e, stackTrace: stackTrace, tag: 'CONNECTION');
  ...
  void _scheduleReconnection() {
    ...
    connectionStatus = 'Reconectando (${_reconnectionAttempts}/$MAX_RECONNECTION_ATTEMPTS)...';
    onConnectionChanged();
  ...
  void _startConnectionMonitoring() {
    _connectionCheckTimer?.cancel();
    _connectionCheckTimer = Timer.periodic(CONNECTION_CHECK_INTERVAL, (timer) async {
      if (!isConnected || _connection == null || isSimulatorMode) return;

      try {
        // Verificar conexión enviando comando simple
        final response = await ELM327Communication.sendCommand(_connection!, "ATI");
  ...
  void setAutoReconnect(bool enabled) {
    ...
    print("🔄 Reconexión automática: ${enabled ? 'HABILITADA' : 'DESHABILITADA'}");
  }
