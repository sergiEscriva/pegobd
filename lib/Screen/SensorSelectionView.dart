...
import '../utils/OBDSensorDetector.dart';
...
class _SensorSelectionViewState extends State<SensorSelectionView> {
  ...
  Future<void> _loadSavedSensors() async {
    ...
    List<String> savedSensors = await SharedPreferencesHelper.getSelectedSensors();
  ...
  Future<void> _scanForAvailableSensors() async {
    ...
    try {
      ...
      Map<String, bool> detected = await OBDSensorDetector.detectAvailableSensors(
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      body: Column(
        children: [
          ...
          Container(
            ...
            color: widget.connectionManager.isConnected ? Colors.green[100] : Colors.red[100],
                  ...
                  children: [
                    Icon(
                      widget.connectionManager.isConnected
                        ? Icons.check_circle
                        : Icons.error,
                      color: widget.connectionManager.isConnected
                        ? Colors.green
                        : Colors.red,
                    ),
                    SizedBox(width: 8),
              ...
              children: [
                Expanded(
                  child: ElevatedButton.icon(
                    onPressed: widget.connectionManager.isConnected
                      ? _scanForAvailableSensors
                      : null,
          ...
          // Lista de sensores
          Expanded(
            child: availableSensors.isEmpty
              ? _buildEmptyState()
              : ListView.builder(
                    ...
                    return Card(
                      ...
                      child: ListTile(
                        ...
                        subtitle: Column(
                          ...
                          children: [
                            Text('PID: $pid'),
                            Text('Unidad: ${ObdSensors.getSensorUnit(pid)}'),
                        ...
                        onTap: isAvailable
                          ? () {
                              setState(() {
                                selectedSensors[pid] = !(selectedSensors[pid] ?? false);
                              });
                            }
                          : null,
                      ),
      ...
      floatingActionButton: _hasSelectedSensors()
  ...
  IconData _getSensorIcon(String pid) {
    switch (pid) {
      case '04': return Icons.flash_on; // Load
      case '05': return Icons.device_thermostat; // Coolant temp
      case '0C': return Icons.speed; // RPM
      case '0D': return Icons.directions_car; // Speed
      case '0F': return Icons.ac_unit; // Intake temp
      case '10': return Icons.air; // MAF
      case '11': return Icons.tune; // Throttle
      case '2F': return Icons.local_gas_station; // Fuel level
      case '5C': return Icons.oil_barrel; // Oil temp
  ...
  Future<void> _saveSensorSelection() async {
    List<String> selected = selectedSensors.entries
